...
class CelebA:
    ...
    train_transform: transforms.transforms.Compose = \
        transforms.Compose([
            ...
            transforms.RandomErasing(0.05, value=mean),
        ])
...
class UTKface:
    ...
    train_transform: transforms.transforms.Compose = \
        transforms.Compose([
            ...
            # transforms.Normalize(mean, std),
            # becauuse of this, random erasing pixel as imagenet mean, not zero
            transforms.RandomErasing(0.2, value=mean),
        ])
    val_transform: transforms.transforms.Compose = \
        transforms.Compose([
            transforms.Resize((224, 224)), 
    ...
    # no partition of dataset provided, ramdom split it into two set
    if race_def == 'FairFace':
        dataset = UTKfaceDataset(root_dir=root, transform=None, race_def='FairFace')
    else:
        dataset = UTKfaceDataset(root_dir=root, transform=None)
...
class FairFace:
    ...
    # dataset & dataloader
    if race_def == 'UTKface':
        train_dataset = FairFaceDataset(train_csv, root, transform=train_transform, race_def='UTKface')
        val_dataset = FairFaceDataset(val_csv, root, transform=val_transform, race_def='UTKface')
    else:
        train_dataset = FairFaceDataset(train_csv, root, transform=train_transform)
        val_dataset = FairFaceDataset(val_csv, root, transform=val_transform)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, \
        shuffle=True, num_workers=12, pin_memory=True, drop_last=True,)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, \
        shuffle=True, num_workers=12, pin_memory=True, drop_last=True,)

...
def main(args):
    ...
    ckpt_path = Path(args.ckpt_root) / args.model_name
    model, optimizer, scheduler = load_model(model, optimizer, scheduler, args.ckpt, ckpt_path)
    ...
    def train():
        ...
        for batch_idx, (data, label) in enumerate(train_dataloader):
            ...
            if args.fairness_matrix == 'equality of opportunity':
                loss = loss_EOpp_perturbed_adaptive(logit, label, sens, p_coef)
            elif args.fairness_matrix == 'equalized odds':
                loss = EO_dy(logit, label, sens, p_coef, n_coef)
            else:
    ...
    # get the current TPR and TNR result
    def get_tprtnr_per_epoch(stat):
        # Input: stats for a single epochs, shape (1, attributes, 8)
        mtp, mfp, mfn, mtn = [stat[0,:,i] for i in range(0, 4)]
        ftp, ffp, ffn, ftn = [stat[0,:,i] for i in range(4, 8)]
        # accuracy
        ...
        mtpr, mtnr = mtp/(mtp+mfn), mtn/(mtn+mfp)
        ftpr, ftnr = ftp/(ftp+ffn), ftn/(ftn+ffp)
        tpr_gap, tnr_gap = abs(mtpr-ftpr), abs(mtnr-ftnr)
        stat_dict = {"total_acc": tacc, "tpr_gap": tpr_gap, "tnr_gap": tnr_gap}
        return stat_dict
    ...
    stat_dict = get_tprtnr_per_epoch(val_stat_per_epoch)
    if args.fairness_matrix == 'equality of opportunity':
        init_tacc, last_pstus = stat_dict['total_acc'], stat_dict['tpr_gap']
    elif args.fairness_matrix == 'equalized odds':
        init_tacc, last_pstus, last_nstus = stat_dict['total_acc'], stat_dict['tpr_gap'], stat_dict['tnr_gap']
    else:
        assert False, f'Unsupport fairness matrix'

    # training loop
    for epoch in range(args.start_epoch, args.epochs):
        print(f'Training with p-coef: {p_coef.clone().cpu()}, n-coef: {n_coef.clone().cpu()}')
        ...
        stat_dict = get_tprtnr_per_epoch(val_stat_per_epoch)
        if args.fairness_matrix == 'equality of opportunity':
            curr_tacc, curr_pstus = stat_dict['total_acc'], stat_dict['tpr_gap']
        elif args.fairness_matrix == 'equalized odds':
            curr_tacc, curr_pstus, curr_nstus = stat_dict['total_acc'], stat_dict['tpr_gap'], stat_dict['tnr_gap']
        else:
            assert False, f'Unsupport fairness matrix'
        for i in range(1):
            if curr_tacc[i] < init_tacc[i] - args.quality_target:
                p_coef[i] = p_coef[i]*1.2
                n_coef[i] = n_coef[i]*1.2
            elif curr_pstus[i] > args.fairness_target and curr_pstus[i] > last_pstus[i]:
                p_coef[i] = p_coef[i]*0.8
                if p_coef[i] < 1e-6:
                    p_coef[i] = 1e-6
            elif args.fairness_matrix == 'equalized odds' and curr_nstus[i] > args.fairness_target and curr_nstus[i] > last_nstus[i]:
                n_coef[i] = n_coef[i]*0.8 
                if n_coef[i] < 1e-6:
                    n_coef[i] = 1e-6
        last_pstus = curr_pstus
        last_nstus = curr_nstus
        #
...
def get_args():
    ...
    parser.add_argument("--ckpt-root", default='/tmp2/aislab/makila/model_checkpoint', type=str, help='root path for model checkpoint')
    parser.add_argument("--stat-root", default='/tmp2/aislab/makila/model_stats', type=str, help='root path for model statistic')
    parser.add_argument("--model-name", default='default', type=str, help='folder name for this model trained')

    parser.add_argument("--ckpt", default="61845_UTKFaceAge_0020", help="name of a checkpoint, without .pth")
